...
const CaregiverSidebar: React.FC<SidebarProps> = ({ activePage, onSelectPage }) => {
  // Filter out Profile and Settings from main pages
  const mainPages = caregiverPages.filter(page => page.label !== "Profile" && page.label !== "Settings");
  const profilePage = caregiverPages.find(page => page.label === "Profile");
  const settingsPage = caregiverPages.find(page => page.label === "Settings");

  return (
    <aside className="w-64 bg-white border-r border-gray-200 p-4 flex flex-col">
      <div className="mb-8">
        <h2 className="text-xl font-bold text-gray-800">Caregiver Portal</h2>
